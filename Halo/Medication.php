...
echo "<div class='row'>";
echo "<div class='col-md-2 head'>";
echo "<a href='editmedication.php?function=new&patid=$ID' class='btn btn-success btn-sm'><i class='plus'></i> Add new line</a>";

echo "</div>
        </div>";
echo "<table class='table-sml table-striped table-bordered'>";
...
while ($row = mysqli_fetch_row($stmt)) {
     echo "<tr>";
     for ($j = 0; $j < 2; $j++) { // we're expecting three attributes
          echo "<td>".$row[$j]."</td>"; // gives the current item of the current attribute
     }

     echo "  <td><a href='editmedication.php?function=edit&id=$row[2]&patid=$ID' class='btn btn-warning btn-sm'>Edit</a></td>";
echo '<td><a  class="btn btn-danger btn-sm" onclick="return confirm(\'Are you sure?\')" href="deletemedication.php?function=delete&id='.$row[2].'&patid='.$ID.'">Delete</a></td>' ;
	    echo "</tr>";
}
?></div><?php
echo "</table>";
